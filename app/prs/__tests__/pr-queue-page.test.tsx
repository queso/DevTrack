...
function makeApiPR(
  ...
  return {
    ...
    check_status: overrides.check_status ?? "passing",
    branch_id: overrides.branch_id ?? "feat/add-tests",
...
describe("AC1: PR list from live API via SWR", () => {
  ...
  it("renders branch name for each PR row", async () => {
    mockUsePRsReturn.data = [
      makeApiPR({ id: "pr-b1", branch_id: "feature/my-branch-name", title: "feat: branch test" }),
    ]
...
describe("Integration: PR row fields wired from mapPR", () => {
  it("renders project slug, title, branch, status, check status, and author for a PR", async () => {
    mockUsePRsReturn.data = [
      makeApiPR({
        ...
        branch_id: "feat/full-row",
        url: "https://github.com/org/repo/pull/55",
        author: "fullauthor",
        opened_at: new Date(Date.now() - 1 * 60 * 60 * 1000), // 1h ago -> green
      }),
    ]
  ...
  it("renders check status badge for passing, failing, and pending states", async () => {
    mockUsePRsReturn.data = [
      makeApiPR({ id: "pr-pass", number: 1, title: "passing PR", check_status: "passing" }),
      makeApiPR({ id: "pr-fail", number: 2, title: "failing PR", check_status: "failing" }),
      makeApiPR({ id: "pr-pend", number: 3, title: "pending PR", check_status: "pending" }),
    ]
...
describe("Edge cases: age calculation boundaries", () => {
  it("PR at exactly 24h renders as '1d' with amber color (not green)", async () => {
    ...
    mockUsePRsReturn.data = [
      makeApiPR({ id: "pr-24h", title: "exactly 24h PR", opened_at: exactlyTwentyFourHoursAgo }),
    ]
  ...
  it("PR at exactly 72h renders as '3d' with amber color (boundary: 3d is still yellow)", async () => {
    ...
    mockUsePRsReturn.data = [
      makeApiPR({ id: "pr-72h", title: "exactly 72h PR", opened_at: exactlySeventyTwoHoursAgo }),
    ]
  ...
  it("PR at 97h (>3 full days) renders as '4d' with red color", async () => {
    ...
    mockUsePRsReturn.data = [
      makeApiPR({ id: "pr-97h", title: "97h PR", opened_at: ninetySevenHoursAgo }),
    ]
  ...
  it("two PRs with identical createdAt timestamps both render (stable sort)", async () => {
    ...
    mockUsePRsReturn.data = [
      makeApiPR({ id: "pr-twin-a", number: 10, title: "twin PR alpha", opened_at: sameTime }),
      makeApiPR({ id: "pr-twin-b", number: 11, title: "twin PR beta", opened_at: sameTime }),
    ]
