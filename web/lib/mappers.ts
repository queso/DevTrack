...
export function mapPR(pr: ApiPullRequest, projectSlug?: string): UiPullRequest {
  return {
    id: pr.id,
    projectSlug: pr.project?.name ?? projectSlug ?? "",
    number: pr.number,
    title: pr.title,
    branch: pr.branch?.name ?? pr.branchId ?? "",
    status: mapPRStatus(pr.status),
    checkStatus: pr.checkStatus ?? "pending",
    createdAt: toISOString(pr.openedAt),
    url: pr.url,
    author: pr.author,
...
function computeActionNeeded(project: ApiProject): boolean {
  ...
  for (const pr of project.pullRequests) {
    ...
    if (pr.checkStatus === "failing") return true
...
function computeCheckStatus(pullRequests: ApiPullRequest[]): UiCheckStatus {
  ...
  if (open.some((pr) => pr.checkStatus === "failing")) return "failing"
  if (open.some((pr) => pr.checkStatus === "pending" || pr.checkStatus == null)) return "pending"
  return "passing"
}
