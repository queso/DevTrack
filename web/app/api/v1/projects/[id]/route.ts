...
// UUID v4 pattern for distinguishing IDs from names
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

function projectWhere(idOrName: string) {
  return UUID_RE.test(idOrName) ? { id: idOrName } : { name: idOrName }
}
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  const { id } = await params
  const existing = await prisma.project.findUnique({ where: projectWhere(id) })
...
export async function DELETE(request: Request, { params }: RouteContext) {
  ...
  try {
    await prisma.project.delete({ where: projectWhere(id) })
