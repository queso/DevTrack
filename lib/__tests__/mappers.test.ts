...
function makePR(overrides: Partial<ApiPullRequest> = {}): ApiPullRequest {
  return {
    ...
    opened_at: msAgo(6 * HOUR),
    merged_at: null,
    created_at: msAgo(6 * HOUR),
    updated_at: msAgo(HOUR),
    ...overrides,
  }
}
...
describe("mapPR", () => {
  ...
  it("maps check_status: passing", () => {
    const result = mapPR(makePR({ check_status: "passing" }), "proj")
    expect(result.checkStatus).toBe("passing")
  })

  it("maps check_status: failing", () => {
    const result = mapPR(makePR({ check_status: "failing" }), "proj")
    expect(result.checkStatus).toBe("failing")
  })

  it("maps null check_status to pending", () => {
    const result = mapPR(makePR({ check_status: null }), "proj")
    expect(result.checkStatus).toBe("pending")
  })

  it("computes age label for PR opened less than 24h ago", () => {
    const pr = makePR({ opened_at: msAgo(3 * HOUR) })
...
describe("mapProject — actionNeeded flag", () => {
  ...
  it("is set when a PR has failing checks", () => {
    const p = makeApiProject({
      pullRequests: [makePR({ status: "open", check_status: "failing" })],
    })
    const result = mapProject(p)
    expect(result.actionNeeded).toBe(true)
  })
  ...
  it("is not set when PR is approved with passing checks", () => {
    const p = makeApiProject({
      pullRequests: [makePR({ status: "approved", check_status: "passing" })],
      lastActivityAt: msAgo(HOUR),
    })
    const result = mapProject(p)
    expect(result.actionNeeded).toBe(false)
...
describe("mapProject — openPRCount and checkStatus", () => {
  ...
  it("returns failing checkStatus when any open PR has failing checks", () => {
    const p = makeApiProject({
      pullRequests: [
        makePR({ status: "open", check_status: "failing" }),
        makePR({ id: "pr-2", status: "open", check_status: "passing" }),
      ],
    })
    const result = mapProject(p)
    expect(result.checkStatus).toBe("failing")
  ...
  it("returns passing when all open PRs pass", () => {
    const p = makeApiProject({
      pullRequests: [makePR({ status: "open", check_status: "passing" })],
    })
    const result = mapProject(p)
    expect(result.checkStatus).toBe("passing")
  })
  ...
  it("returns pending when a PR has pending checks", () => {
    const p = makeApiProject({
      pullRequests: [makePR({ status: "open", check_status: "pending" })],
    })
    const result = mapProject(p)
    expect(result.checkStatus).toBe("pending")
  })
...
describe("mapPR — edge cases", () => {
  ...
  it("handles opened_at as a string (API JSON deserialization)", () => {
    // BUG PROBE: mapPR calls pr.opened_at.toISOString(), but when data comes
    ...
    const prWithStringDate = {
      ...makePR(),
      opened_at: "2026-03-10T00:00:00.000Z" as unknown as Date,
    }
